...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i < num_rolls:
        ...
        total += roll
        i += 1
    return total if not pig_out else 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        return int(1 + max(opponent_score % 10, (opponent_score - (opponent_score % 10)) / 10))
    else:
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while game_is_on:
        if who == 0:
            ...
            opponent_score += take_turn(strategy1(score, opponent_score), score, select_dice(score, opponent_score))
            score, opponent_score = swine_swap(score, opponent_score)
        who = other(who)
        if goal <= score or goal <= opponent_score:
            game_is_on = False
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    avg = 0
    for i in range(1, 11):
        avg = make_averaged(roll_dice, 1)(i, dice)
        print(f'{i} dice scores ' + str(avg) + ' on average')
    print(10)
...
def bacon_strategy(score, opponent_score):
    ...
    0
    """
    result = take_turn(0, opponent_score, select_dice(score, opponent_score))
    if result >= BACON_MARGIN:
        return 0
    else:
...
def swap_strategy(score, opponent_score):
    ...
    """
    result = take_turn(0, opponent_score, select_dice(score, opponent_score))
    if score + result == opponent_score / 2:
        return 0
    elif (score + result) / 2 == opponent_score:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    if total + take_turn(swap_strategy(score, opponent_score), opponent_score, select_dice(score, opponent_score)) % 7 == 0:
        return 5
